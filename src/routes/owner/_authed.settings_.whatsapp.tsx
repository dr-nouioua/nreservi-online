...
function WhatsappSettingsPage() {
  ...
  return (
    <div className="max-w-3xl space-y-6 p-6 lg:p-8">
      <div>
        <Link to="/owner/settings" className="inline-flex items-center gap-1 text-sm text-stone-500 hover:text-stone-800">
          <ArrowLeft className="h-3.5 w-3.5" /> Paramètres
        </Link>
        <h1 className="mt-2 flex items-center gap-2 text-3xl font-bold tracking-tight text-stone-950">
      ...
      <form onSubmit={submitNumber} className="space-y-3 rounded-lg border border-stone-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="font-semibold text-stone-900">Numéro WhatsApp</p>
          {savedNumber && (
        ...
        <div className="flex flex-wrap gap-2">
          <input
            ...
            inputMode="tel"
            autoComplete="tel"
            className="min-w-0 flex-1 rounded-lg border border-stone-300 px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none"
          />
          <button
            disabled={savingNumber}
            className="inline-flex items-center gap-2 rounded-lg bg-stone-950 px-4 py-2 text-sm font-medium text-white hover:bg-stone-800 disabled:opacity-60"
          >
      ...
      <div className="rounded-lg border border-stone-200 bg-white p-6 shadow-sm">
        <p className="font-semibold text-stone-900">Variables disponibles</p>
        ...
        <ul className="mt-3 flex flex-wrap gap-2">
          {WHATSAPP_VARIABLES.map((v) => (
            <li key={v.token} className="rounded-full bg-stone-100 px-3 py-1 text-xs text-stone-600">
              <code className="font-mono text-stone-800">{v.token}</code> — {v.label}
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="space-y-4">
        <p className="font-semibold text-stone-900">Messages par défaut</p>
  ...
  return (
    <div className="rounded-lg border border-stone-200 bg-white p-6 shadow-sm">
      ...
      <textarea
        ...
        rows={10}
        className="mt-3 w-full rounded-lg border border-stone-300 px-3 py-2 font-mono text-xs leading-relaxed text-stone-800 focus:ring-2 focus:ring-emerald-500 focus:outline-none"
      />
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <button
          type="button"
          onClick={save}
          className="inline-flex items-center gap-2 rounded-lg bg-stone-950 px-4 py-2 text-sm font-medium text-white hover:bg-stone-800"
        >
          <Save className="h-4 w-4" /> Enregistrer
        </button>
        <button
          ...
          onClick={restore}
          disabled={isDefault}
          className="inline-flex items-center gap-2 rounded-lg border border-stone-300 px-3 py-2 text-sm text-stone-700 hover:bg-stone-50 disabled:opacity-50"
        >
      ...
      {showPreview && (
        <pre className="mt-3 rounded-lg bg-stone-50 px-3 py-2 text-xs whitespace-pre-wrap text-stone-700">
